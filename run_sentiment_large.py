...
from env.sentiment_large_env import SentimentPipelineEnv
...
# =============================================================================
CONFIG = {
    # Train 3 separate agents: One on WRS, one on DIE, one on BOTH
    "ACTIVE_MODES": ["WRS", "DIE", "BOTH"], 
    "TRAIN_EPISODES": 500,
    "EVAL_EPISODES": 500,
    "TRACE_LOG_EPISODES": 500, 
    "DATA_PATH": os.path.join(root, "composite-rating-rddl", "data", "input", "real_world", "master_sentiment_allure.csv"),
...
def run_experiment_full_trace():
    ...
    # Initialize Env (Default start mode)
    ...
    # Trace File Setup
    csv_trace_path = "agent_trace_metrics_full.csv"
    ...
    print("\n=== Trace Generation Complete (Full Suite) ===")
    ...
    df.to_csv("sentiment_results_table_full.csv", index=False)
